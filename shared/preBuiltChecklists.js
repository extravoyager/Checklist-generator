// Shared pre-built EHS checklist library. Imported by both client and server.
...
function q(text, opts = {}) {
  return {
    ...
    criticality: opts.criticality || 'standard',
...
export function buildPreBuiltChecklists() {
  _seq = 0
  ...
  return list.map((t, i) => {
    ...
    const questionCount = sections.reduce((acc, sec) => acc + sec.questions.length, 0)
    return {
...
export const preBuiltChecklists = buildPreBuiltChecklists()
